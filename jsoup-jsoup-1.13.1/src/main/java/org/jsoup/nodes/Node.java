...
public abstract class Node implements Cloneable {
	...
	public int getXPathCode() {
		return xpathCode;
	}

	private void setXPathCode() {
		if (parentNode != null) {
			xpathCode = parentNode.getXPathCode();
		}
		String xpath = siblingIndex + XPATH_SEPARATOR;
		int h = xpathCode;
		for (int i = 0; i < xpath.length(); i++) {
			h = 31 * h + xpath.charAt(i);
		}
		setXPathCode(h);
	}

	public void setXPathCode(int xpathCode) {
	...
	public String getXPath() {
		...
		while (node.parentNode != null) {
			node = node.parentNode;
			xpath = node.siblingIndex + XPATH_SEPARATOR + xpath;
	...
	public void setCharEndPos(int charEndPos) {
		this.charEndPos = charEndPos;
	}
	...
	public void setByteStartPos(int byteStartPos) {
	...
	public boolean validNode(Node child) {
		if (child instanceof Comment || child instanceof DocumentType) {
			return false;
		}
		if (child instanceof TextNode && parent() == null) {
			return false;
		}
		return true;
	}
